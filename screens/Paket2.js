...
const Paket2 = () => {
  ...
  //-----------------------------------------------------------------
  const [urun2, setUrun2] = useState('null')
  id= '2'
  const paket2Doc = doc(db, "urun", id )
  getDoc(paket2Doc)
  // Handling Promises
  .then((snapshot) => {
    // MARK: Success
    if (snapshot.exists) {
      setUrun2(snapshot.data())
    }
    else {
...
<View style={styles.yazi}>
               ...
               <Text style={styles.baslik}> PAKET 2</Text>
                        <Text style={styles.metin}>   
                        2. Paketi almanız durumunda seçtiğiniz tarihlere göre  "Haftada 2 kere yani ayda 8 kere yıkama ve cilalama işlemine tabi tutuyoruz." 
                          </Text>
        ...
        <View style={styles.kabin }>
          ...
          <Text style={styles.title}> {urun2.paket}</Text>
                   <Text style={styles.paragraf}>    {urun2.yikama}
                     </Text>
                         <Text style={styles.paragraf}>                            + </Text>
                         <Text style={styles.paragraf}>                    Cilalama </Text>
                         <Text style={styles.paragraf}>                       {urun2.fiyat} TL</Text>
