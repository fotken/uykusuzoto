...
const Paket4 = () => {
  ...
  //-----------------------------------------------------------------
  const [urun4, setUrun4] = useState('null')
  id= '4'
  const paket4Doc = doc(db, "urun", id )
  getDoc(paket4Doc)
  // Handling Promises
  .then((snapshot) => {
    // MARK: Success
    if (snapshot.exists) {
      setUrun4(snapshot.data())
    }
    else {
      alert("No Doc Found")
    }
...
<View style={styles.yazi}>
               ...
               <Text style={styles.baslik}> PAKET 4</Text>
                        <Text style={styles.metin}>   
                        4. Paketi almanız durumunda seçtiğiniz tarihlere göre  "1 Ayda hergün yıkama ve cilalama işlemine tabi tutuyoruz." 
                          </Text>
        ...
        <View style={styles.kabin }>
          ...
          <Text style={styles.title}> {urun4.paket}</Text>
                   <Text style={styles.paragraf}>    {urun4.yikama}
                     </Text>
                         <Text style={styles.paragraf}>                             + </Text>
                         <Text style={styles.paragraf}>                     Cilalama </Text>
                         <Text style={styles.paragraf}>                       {urun4.fiyat} TL</Text>
