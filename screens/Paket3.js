...
const Paket3 = () => {
  ...
  //-----------------------------------------------------------------
  const [urun3, setUrun3] = useState('null')
  id= '3'
  const paket3Doc = doc(db, "urun", id )
  getDoc(paket3Doc)
  // Handling Promises
  .then((snapshot) => {
    // MARK: Success
    if (snapshot.exists) {
      setUrun3(snapshot.data())
    }
    else {
      alert("No Doc Found")
    }
...
<View style={styles.yazi}>
               ...
               <Text style={styles.baslik}> PAKET 3</Text>
                        <Text style={styles.metin}>   
                        3. Paketi almanız durumunda seçtiğiniz tarihlere göre  "Haftada 3 kere yani ayda 12 kere yıkama ve cilalama işlemine tabi tutuyoruz." 
                          </Text>
        ...
        <View style={styles.kabin }>
          ...
          <Text style={styles.title}> {urun3.paket}</Text>
                   <Text style={styles.paragraf}>    {urun3.yikama}
                     </Text>
                         <Text style={styles.paragraf}>                            + </Text>
                         <Text style={styles.paragraf}>                      Cilalama </Text>
                         <Text style={styles.paragraf}>                       {urun3.fiyat} TL</Text>
