...
const BayiHesap = () => {
  ...
  const handleCreateAccount = () => {
    ...
    .then((userCredential) => {
      ...
      {user ? navigation.navigate('BayiKayit', {user}) : navigation.navigate('Hesabim')} 
    })
    .catch(error => {
      console.log(error)
  ...
  const handleSignIn = () => {
    ...
    .then((userCredential) => {
      ...
      {user ? navigation.navigate('BayiProfil', {user} ) : navigation.navigate('Hesabim')} 
    })
    .catch(error => {
      console.log(error)
    })
          ...
          <BlurView intensity={100}>
            ...
            <View style={styles.login}>
              ...
              <TouchableOpacity onPress={handleSignIn} style={[styles.button, {backgroundColor: '#00CFEB90'}]}>
                <Text style={{fontSize: 17, fontWeight: '400', color: 'black'}}>Bayi Girişi</Text>
              </TouchableOpacity>
              <TouchableOpacity  onPress={handleCreateAccount} style={[styles.button, {backgroundColor: '#6792F090'}]}>
                <Text style={{fontSize: 17, fontWeight: '400', color: 'black'}}>Bayi Kayıt Ol</Text>
              </TouchableOpacity>
